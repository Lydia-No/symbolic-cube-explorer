...
def is_lineage_preserved(seq):
    """
    Minimal lineage rule:
    start and end match, and sequence is not trivial.
    """
    if len(seq) < 2:
        return False
    if seq[0] != seq[-1]:
...
def variation_depth(seq):
    """
    Crude variation measure:
    number of axis switches + number of distinct axes - 1
    """
...
def run(total=10000, length=6):
    print("\nLINEAGE THRESHOLD SCAN\n")
    ...
    for info in lineage_examples[:10]:
        print(
